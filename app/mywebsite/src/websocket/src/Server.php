...
use BloatlessModified\WebSocket\Application\ApplicationInterface;

/**
 * Simple WebSocket server implementation in PHP.
 *
 * @author Simon Samtleben <foo@BloatlessModified.org>
...
class Server
{
                ...
                if ($socket == $this->master) {
                    if (($ressource = stream_socket_accept($this->master)) === false) {
                        ...
                        if (count($this->clients) > $this->maxClients) {
                            $client->onDisconnect();
                            if ($this->hasApplication('ws/status')) {
                                $this->getApplication('ws/status')->statusMsg(
    ...
    public function removeClientOnClose(Connection $client): void
    {
        ...
        if ($this->hasApplication('ws/status')) {
            $this->getApplication('ws/status')->clientDisconnected($clientIp, $clientPort);
        }
